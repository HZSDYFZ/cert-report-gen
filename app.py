...
def extract_form_fields(doc):
    ...
    for table in doc.tables:
        for ri, row in enumerate(table.rows):
            ...
            if ri == 4 and len(cells) > 1 and cells[1]:
                fields['auditType'] = cells[1]
            if ri == 5 and len(cells) > 1 and cells[1]:
                fields['address'] = cells[1]
            if ri == 6 and len(cells) > 1 and cells[1]:
                fields['scope'] = cells[1]
...
st.markdown('---')
st.caption('Cert Report Generator v2.6')
