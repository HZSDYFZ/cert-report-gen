        ...
        for ri, row in enumerate(table.rows):
                            ...
                            for run in para.runs:
                                if run.text and "公司名称" in run.text:
                                    ...
                                    for i, r in enumerate(runs):
                                        if r.text and "公司名称" in r.text:
                                            if i + 1 < len(runs): runs[i + 1].text = (runs[i + 1].text or "") + company
                                            else: r.text = r.text + company
                                            filled["company"] = True; break
                    ...
                    for para in cells[3].paragraphs:
                        for run in para.runs:
                            if run.text and ("任务号" in run.text or "任务编号" in run.text):
                                runs = list(para.runs)
                                for i, r in enumerate(runs):
                                    if r.text and ("任务号" in r.text or "任务编号" in r.text):
                                        if i + 1 < len(runs): runs[i + 1].text = (runs[i + 1].text or "") + taskNo
                                        else: r.text = r.text + taskNo
                                        filled["taskNo"] = True; break
                                if filled["taskNo"]: break
                        if filled["taskNo"]: break
            if ri == 1 and leader and not filled["leader"]:
                for ci in range(2, min(5, len(cells))):
                    ...
                    for para in cells[ci].paragraphs:
                        if para.runs:
                            ...
                            # Cell has no runs - add text directly
                            para.add_run(leader)
                            filled["leader"] = True
                            found = True
                            break
            ...
            if ri == 2:
                for ci in range(2, min(5, len(cells))):
                    ...
                    if "ISO" in cell_text or "IATF" in cell_text:
                        new_text = fill_cert_standard(cell_text, auditType)
                        if new_text != cell_text:
                            cells[ci].text = new_text; break
            if ri == 3 and auditType:
                for ci in range(2, min(5, len(cells))):
                    cell_text = cells[ci].text
                    if "初审" in cell_text or "监审" in cell_text or "再认证" in cell_text:
                        new_text = fill_audit_type(cell_text, auditType)
                        if new_text != cell_text:
                            cells[ci].text = new_text; break
...
def parse_and_fix_excel(file_buffer):
    ...
    for idx, row in df1.iterrows():
        ...
        if _is_empty(task_no) and not df2.empty and "任务号" in df2.columns:
            ...
            if s1_company:
                mask = (df2["企业名称"].astype(str).str.contains(s1_company[:8], na=False, regex=False) |
                        df2["企业中文名字"].astype(str).str.contains(s1_company[:8], na=False, regex=False))
                match = df2[mask]
                if not match.empty:
...
def generate_word_zip_batch(df, template_bytes):
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        for idx, row in df.iterrows():
            ...
            raw_company = str(data_dict.get("公司中文名", "企业_%d" % (idx + 1)))
            company_name = re.sub(r"[\\/*?:\"<>|]", "_", raw_company)
            task_no = re.sub(r"[\\/*?:\"<>|]", "_", str(data_dict.get("任务号", "")))
            filename = "%s_%s_评定报告.docx" % (company_name, task_no) if task_no else "%s_评定报告.docx" % company_name
