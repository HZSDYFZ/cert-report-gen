...
def extract_form_fields(doc):
    ...
    for para in doc.paragraphs:
        ...
        if '组织名称' in t or '公司' in t: fields['company']=re.split(r'[:：]',t)[-1].strip()
...
st.set_page_config(page_title='Cert Report Generator', page_icon='\U0001f4cb', layout='wide')
...
if mode=='Single Report':
    ...
    if ff and tf:
        ...
        st.info(f"Extracted: company={f.get('company','')}, taskNo={f.get('taskNo','')}, leader={f.get('leader','')}")
        if st.button('Generate Report',type='primary',key='gen_s'):
            with st.spinner('Generating...'):
                try:
                    ...
                    st.download_button(label='Download Report',data=out.getvalue(),file_name=f"{f.get('company','report')}.docx",mime='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
    ...
    if ef and tf:
        ...
        st.progress(prog)
        st.write(f'Progress: {processed}/{total}')
        if st.button('Start Generation (20 at a time)',key='b_start'):
            with st.spinner('Generating...'):
                ...
                if new:
                    ...
                    st.success(f'Generated {len(new)} reports (total: {st.session_state.batch_processed}/{total}')
                    st.download_button(label='Download Batch (ZIP)',data=st.session_state.curr_zip,file_name=f'reports_batch_{step+1}.zip',mime='application/zip')
                    if st.session_state.batch_processed>=total:
                        ...
                        if len(st.session_state.batch_files)>1:
                            ...
                            azb.seek(0)
                            st.download_button(label='Download All (ZIP)',data=azb.getvalue(),file_name='all_reports.zip',mime='application/zip')
